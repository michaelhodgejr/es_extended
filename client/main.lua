...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  ...
  local jobTpl = '<div>{{job_label}}</div>'
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0, Keys["~"]) and not ESX.UI.Menu.IsOpen('default', 'es_extended', 'inventory') and (GetGameTimer() - GUI.Time) > 150 then
      ESX.ShowInventory()
...
RegisterNetEvent('esx:playerLoaded')
AddEventHandler('esx:playerLoaded', function(xPlayer)
  TriggerServerEvent("esx:updateConnection")
end)
