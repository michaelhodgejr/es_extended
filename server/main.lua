...
AddEventHandler('esx:onPickup', function(id)
  ...
  if pickup.type == 'item_standard' then
    ...
    if remaining > 0 then
      TriggerClientEvent('esx:showNotification', _source, 'You have no more room for ~b~' .. item.label .. '~s~')
...
RegisterServerEvent('esx:updateConnection')
AddEventHandler('esx:updateConnection', function()
  local xPlayer        = ESX.GetPlayerFromId(source)
  local ostime = os.date('%c', os.time())
  print("Lastconnection - updated "..ostime.." on "..xPlayer.identifier)

  MySQL.Async.execute(
        'UPDATE users SET lastconnection = NOW() WHERE identifier = @identifier',
        {
          ['@identifier'] = xPlayer.identifier,
        },
